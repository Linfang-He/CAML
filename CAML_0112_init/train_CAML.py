...
class CFExperiment(Experiment):
    ...
    def _prepare_set(self, data):
        ...
        if self.args.implicit == 1:
            ...
            idx += 1
            self.mdl.register_index_map(idx, 'user_indices')
            idx += 1
            self.mdl.register_index_map(idx, 'item_indices')
    ...
    def _load_sets(self):
        ...
        # rating_set是user_id item_indices rating三元组的集合
        # reviews是review的集合，review是用向量表示的（word->index）
        ...
        if (self.no_text_mode == False):
            # load_reviews
            print('训练数据 self.load_review_data review...')
            self.ui_review_dict, self.iu_review_dict = self.load_review_data(data_link, "review")  # ui_dict[user][item]
            print('训练数据 self.load_review_data concepts...')
