...
class Model:
    ...
    def _get_pair_feed_dict(self, data, mode='training', lr=None):
        ...
        if (self.args.features):
            ...
            if (mode == 'training'):  # 训练模式下需要负样本，如果删除评分部分，就不需要负样本
                feed_dict[self.neg_features] = data[7]
        return feed_dict

    # 训练方式 point-wise/pair-wise/list-wize
    ...
    def _get_point_feed_dict(self, data, mode='training', lr=None):
        ...
        if self.args.implicit == 1:
            feed_dict[self.user_id] = data[self.imap['user_indices']]
            feed_dict[self.item_id] = data[self.imap['item_indices']]
        if (mode != 'infer'):
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope("train"):
                ...
                # save model
                self.saver = tf.compat.v1.train.Saver(tf.trainable_variables(), max_to_keep=0)
    ...
    def _beam_search_infer(self, q1_output, q2_output, r_input, reuse=None):
        ...
        with tf.variable_scope('gen_review', reuse=reuse) as scope:
            ...
            if r_input is not None:
                ...
                state = tf.nn.tanh(r_embed + tf.matmul(q1_output, self.review_user_mapping) + tf.matmul(q2_output,
                                                                                                        self.review_item_mapping) + self.review_bias)
            else:
                state = tf.nn.tanh(tf.matmul(q1_output, self.review_user_mapping) + tf.matmul(q2_output,
                                                                                              self.review_item_mapping) + self.review_bias)
            ...
            neg_words = tf.reshape(tf.scatter_update(neg_words, tf.constant(self.UNK_tag), tf.constant(1.0)),
            ...
            init_state = tf.reshape(tf.tile(state, [1, self.args.beam_size]),
                                    shape=[self.beam_batch, self.args.rnn_dim])
            ...
            def forward_one_step(end_tag, tag, answer, lens, loss, lm_inputs, state):
                ...
                loss_new = tf.reshape(
                    tf.tile(tf.reshape(loss_new, shape=[self.beam_batch, 1]), [1, self.args.beam_size]),
                    shape=[self.beam_batch_max])
                ...
                lens_new = tf.reshape(tf.tile(tf.reshape(lens, shape=[self.beam_batch, 1]), [1, self.args.beam_size]),
                                      shape=[self.beam_batch_max])
                lens_new = lens_new + 1

                # merge
                merge_tag = tf.concat([tf.reshape(tag_old, shape=[self.args.batch_size, -1]),
                                       tf.reshape(tag_new, shape=[self.args.batch_size, -1])], axis=1)
                merge_lens = tf.concat([tf.reshape(lens_old, shape=[self.args.batch_size, -1]),
                                        tf.reshape(lens_new, shape=[self.args.batch_size, -1])], axis=1)
                merge_state = tf.concat([tf.reshape(state_old, shape=[self.args.batch_size, self.args.beam_size, -1]),
                                         tf.reshape(state_new, shape=[self.args.batch_size,
                                                                      self.args.beam_size * self.args.beam_size, -1])],
                                        axis=1)
                merge_loss = tf.concat([tf.reshape(loss_old, shape=[self.args.batch_size, -1]),
                                        tf.reshape(loss_new, shape=[self.args.batch_size, -1])], axis=1)
                # average_loss = tf.div(merge_loss, merge_lens)
                merge_answer = tf.concat([tf.reshape(answer_old, shape=[self.args.batch_size, self.args.beam_size, -1]),
                                          tf.reshape(answer_new, shape=[self.args.batch_size,
                                                                        self.args.beam_size * self.args.beam_size,
                                                                        -1])], axis=1)
                merge_inputs = tf.concat([tf.reshape(lm_inputs, shape=[self.args.batch_size, -1]),
                                          tf.reshape(indices, shape=[self.args.batch_size, -1])], axis=1)
                ...
                range_batch = tf.reshape(
                    tf.tile(tf.reshape(tf.range(self.args.batch_size), shape=[self.args.batch_size, 1]),
                            [1, self.args.beam_size]), shape=[self.beam_batch, 1])
            ...
            sum_end, tag, answer, lens, loss, lm_inputs, state = tf.while_loop(condition, forward_one_step,
                                                                               [init_end_tag, init_tag, init_ans,
                                                                                init_len, init_loss, init_lm_inputs,
                                                                                init_state], shape_invariants=[
                    init_end_tag.get_shape(), init_tag.get_shape(), tf.TensorShape([self.beam_batch, None]),
                    init_len.get_shape(), init_loss.get_shape(), init_lm_inputs.get_shape(), init_state.get_shape()])
