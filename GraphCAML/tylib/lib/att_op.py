...
def co_attention(input_a, input_b, reuse=False, name='', att_type='SOFT',
    ...
    # 这里的input_a input_b是[bs, 20, 50];
    # train中，valid中，input_a中每20个条目是不同的， input_b是不同的
    # valid中，input_a中每20个条目是相同的， input_b是不同的
    ...
    #上述经过映射层后：input_a input_b是[bs, 20, 50];
    #train中，input_a中每20个条目是不同的， input_b是不同的
    #valid中，input_a中每20个条目是相同的， input_b是不同的
    ...
    if(pooling=='MATRIX'):
        ...
        # 更新前的att_col att_row：[bs, 20]；计算得到的也是[bs, 20]
        # 在train中，更新前的att_col att_row的20条之间都是不同的
        # 在valid中，更新前的att_col att_row的20条之间都是相同的
        if(mask_a is not None and mask_b is not None):
