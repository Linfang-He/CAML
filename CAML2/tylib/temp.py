...
class CFExperiment(Experiment):
    ...
    def __init__(self, inject_params=None):
        ...
        self.show_metrics = ['MSE', 'RMSE', 'MAE', 'MSE_int', 'RMSE_int', 'MAE_int', 'Gen_loss', 'All_loss', 'Gen_loss',
                             'F1', 'ACC', 'Review_acc']
        ...
        print('=' * 30, 'Setting up environment', '=' * 30)
        self.model_wrapper()
        self.model_name = self.args.rnn_type
        # self._setup()
        # MODIFIED
        self.out_dir = "cv/test"
        ...
        self.mdl = Model(self.vocab, self.args,
                         ...
                         mode='HREC', num_item=self.num_items_train,
                         num_user=self.num_users_train)
        print('Finish！ time (s):', round(time.time() - t1))
    ...
    # user item rating review review_len的元组
    ...
    def load_dataset(self, data_dir, dataset_type):
        ...
        lines_rating = []
        lines_review = []
        with open("data/%s_data.json" % (dataset_type), 'r') as f:
            for line in f:
                temp = json.loads(line)
                lines_user_id.append(temp['userid'])
                lines_item_id.append(temp['itemid'])
                lines_rating.append(temp['rating'])
                lines_review.append(temp['review'])

        '''
        lines_user_id = codecs.open('%s/%s_userid.txt' % (data_dir, dataset_type), 'rb', 'utf-8').readlines()
        print(lines_user_id)

        lines_item_id = codecs.open('%s/%s_itemid.txt' % (data_dir, dataset_type), 'rb', 'utf-8').readlines() #['103980\r\n', '70426\r\n', '93416\r\n', '45300\r\n',
        ...
        for i in range(len(lines_rating)):
            ...
            # output.append([int(lines_user_id[i]), int(lines_item_id[i]), int(lines_rating[i])])
            # 3.20
            output.append([lines_user_id[i], lines_item_id[i], lines_rating[i]])
            linedata = []
    ...
    def load_review_data(self, data_dir, data_type, dataset_type):
        '''
        lines_item_id = codecs.open('%s/%s_itemid.txt' % (data_dir, dataset_type), 'rb', 'utf-8').readlines()
        lines_user_id = codecs.open('%s/%s_userid.txt' % (data_dir, dataset_type), 'rb', 'utf-8').readlines()
        lines_review = codecs.open('%s/%s_%s.txt' % (data_dir, dataset_type, data_type), 'rb', 'utf-8').readlines()
        '''
        lines_user_id = []
        lines_item_id = []
        lines_review = []
        ...
        for i in range(len(lines_review)):
            # 3.20
            user = lines_user_id[i].strip()
            item = lines_item_id[i].strip()
    ...
    def _load_sets(self):
        # 加载数据 Load train, test and dev sets
        data_link = self.args.data_link

        if (self.no_text_mode == False):
            self.word_index = self.load_vocab(data_link)  # 在后面表示向量时用到
        ...
        # rating_set是user_id item_id rating三元组的集合
        # revieews是review的集合，review是用向量表示的（word->index）
        ...
        if (self.no_text_mode == False):
            # load_reviews
            self.ui_review_dict_train, self.iu_review_dict_train = self.load_review_data(data_link, "review",
                                                                                         "train")  # ui_dict[user][item]
            # self.ui_concept_dict_train, self.iu_concept_dict_train = self.load_review_data(data_link, "concepts", "train")
            self.ui_concept_dict_train, self.iu_concept_dict_train = self.load_review_data(data_link, "review", "train")
            self.num_users_train = len(self.ui_review_dict_train)
            self.num_items_train = len(self.iu_review_dict_train)

            self.ui_review_dict = self.ui_review_dict_train
            self.iu_review_dict = self.iu_review_dict_train
            self.ui_concept_dict = self.ui_concept_dict_train
            self.iu_concept_dict = self.iu_concept_dict_train

            self.ui_review_dict_valid, self.iu_review_dict_valid = self.load_review_data(data_link, "review",
                                                                                         "valid")  # ui_dict[user][item]
            # self.ui_concept_dict_valid, self.iu_concept_dict_valid = self.load_review_data(data_link, "concepts", "valid")
            self.ui_concept_dict_valid, self.iu_concept_dict_valid = self.load_review_data(data_link, "review", "valid")

            self.num_users_valid = len(self.ui_review_dict_valid)
            self.num_items_valid = len(self.iu_review_dict_valid)

            self.ui_review_dict_test, self.iu_review_dict_test = self.load_review_data(data_link, "review",
                                                                                       "test")  # ui_dict[user][item]
            self.ui_concept_dict_test, self.iu_concept_dict_test = self.load_review_data(data_link, "concepts",
                                                                                         "test")
            self.num_users_test = len(self.ui_review_dict_test)
            self.num_items_test = len(self.iu_review_dict_test)
    ...
    def evaluate(self, data, bsz, epoch, name="", set_type=""):
        ...
        # 将str类型的rating转换成int类型的
        actual_labels = [float(x) for x in actual_labels]

        if set_type == "Dev":
            self.ui_review_dict = self.ui_review_dict_valid
            self.iu_review_dict = self.iu_review_dict_valid
            self.ui_concept_dict = self.ui_concept_dict_valid
            self.iu_concept_dict = self.iu_concept_dict_valid
        elif set_type == "Test":
            self.ui_review_dict = self.ui_review_dict_test
            self.iu_review_dict = self.iu_review_dict_test
            self.ui_concept_dict = self.ui_concept_dict_test
            self.iu_concept_dict = self.iu_concept_dict_test
        ...
        for i in tqdm(range(num_batches + 1)):
            ...
            loss, preds, gen_loss, gen_acc, word_att1, word_att2 = self.sess.run([self.mdl.task_cost,
                                                                                  predict_op, self.mdl.gen_loss,
                                                                                  self.mdl.gen_acc,
                                                                                  self.mdl.word_att1,
                                                                                  self.mdl.word_att2], feed_dict)
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            for i in tqdm(range(0, num_batches + 1)):
                ...
                #_, loss = self.sess.run([train_op, self.mdl.cost], feed_dict)
                #print('att1 shape:', att1.shape)
                #print('att2 shape:', att2.shape)
                # print('word_att1 shape:', word_att1.shape)
                # print('word_att2 shape:', word_att2.shape)
            ...
            if (epoch % self.args.eval == 0):
                ...
                stop, max_e, best_epoch = self._select_test_by_dev(
                    ...
                    self.eval_test,
                    lower_is_better=True)
                # 早停法：当验证集精度连续5个epoch不再增加时，则停止训练；也就是说连续5个epoch精度不再提高
        ...
        for i in tqdm(range(0, num_batches + 1)):
            ...
            for j in range(self.args.batch_size):
                if (self.args.batch_size * i + j) < data_len:
                    ...
                    f1 = open(self.args.gen_true_dir + '/true_review.A.' + str(self.args.batch_size * i + j) + '.txt',
                              'w+')
